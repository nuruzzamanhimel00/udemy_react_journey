...
function App() {
  ...
  const fetchDatas = useCallback(async () => {
    ...
    try {
      ...
      const response = await Common.httpRequest({
        url: "http://127.0.0.1:8000/api/get-all-post",
        method: "GET"
      })
      const cartsResponse = await Common.httpRequest({
        url: "http://127.0.0.1:8000/api/fetch-carts",
        method: "GET"
      })
      ...
      const data = await response.datas.data;
      const cartData = await cartsResponse.datas.data;
    ...
    //fetch cart
    cartData.forEach((d) => {
      let findData = data.find((item) => item.id === d.id)
      if (findData) {
